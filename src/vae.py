...
# ---------------------------------------------------------------------------
# Reproducibility: set all random seeds before any model is built.
# NOTE: Full determinism on GPU also requires setting the environment variable
#       PYTHONHASHSEED=42 before launching the Python interpreter and using a
#       single-threaded TF session (intra_op_parallelism_threads=1,
#       inter_op_parallelism_threads=1).  GPU operations may still be
#       non-deterministic even with these settings.
# ---------------------------------------------------------------------------
random.seed(config.RANDOM_SEED)
np.random.seed(config.RANDOM_SEED)
tf.random.set_seed(config.RANDOM_SEED)

# disable_eager_execution() was used in the original to avoid a TF1-style
# training incompatibility.  In TF 2.6 it is generally not required and can
# hide performance improvements, so it is intentionally omitted here.
# Uncomment the two lines below if you observe training instability:
...
class VAE(tf.keras.Model):
    ...
    def determine_classification_threshold(self, x_train):
        ...
        # Optimization: use config constant for threshold percentile
    ...
    def predict_anomaly(self, data):
        ...
        # Optimization: vectorized comparison instead of a Python loop
