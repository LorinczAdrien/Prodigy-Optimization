...
def main(repeat_nums, expConfig_nums, data_dir, pre_selected_features_filename, output_dir, verbose=False):
    ...
    # Reproducibility: set all random seeds at the very start of main()
    ...
    for repeat_num in repeat_nums:
        ...
        for expConfig_num in expConfig_nums:
            ...
            # Set healthy_test_data_percentage based on expConfig_num using config
            ...
            # Reproducibility: use random_state=RANDOM_SEED for deterministic splits
            ...
            # Optimization: use config constants for architecture ratios
            intermediate_dim = int(input_dim / config.INTERMEDIATE_DIM_RATIO)
            latent_dim = int(input_dim / config.LATENT_DIM_RATIO)

            # Memory management: explicitly delete the previous VAE model if it exists
            if 'vae' in locals():
            ...
            # Train the VAE model using config constants
            vae.fit(
            ...
            # Memory management: clean up large DataFrames after each iteration
